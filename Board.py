...
class Board():
    ...
    player1 = 1
    player2 = -1
    # player1 plays left right (y == 0 to y == self.size - 1) and player2 plays top down (x == 0 to x == self.size -1 )
    ...
    def paint_neighbor(self, x, y):
        ...
        for i in self.neighborhood(x, y):
            if self.board[0, i[0], i[1]] == player and self.board[1, i[0], i[1]] == 0:
                self.board[1, i[0], i[1]] = 1
                self.paint_neighbor(i[0], i[1])

    def check_conn_start(self, x, y, player):
        if (x == 0 and player == self.player2) or (y == 0 and player == self.player1):
            self.board[1, x, y] = 1
